...
const PLACEHOLDER_PROJECTS = [
  ...
  },
  {
    ...
    price_range: 'RM 620K–RM 1.068M',
    tenure: 'Freehold',
    commute_note: 'Planned covered route to RTS and CIQ within 850m',
    description: "Summer Suites (Residensi Bukit Meldrum) is a 748-unit freehold serviced apartment in the JB city-centre corridor, with unit sizes from 599 to 912 sq ft and estimated completion in June 2029. Type A is a 3-bedroom dual-key layout, Type B is a conventional 2+1-bedroom home, and Type C is a compact dual-studio layout. A planned covered route connects towards the RTS and CIQ within 850 metres. Its suitability depends on whether the city-centre location and layout flexibility match the buyer's own-stay or investment plan.",
    unit_types: [
    ...
    features: [
      ...
      'Selling prices from RM 620,000 to RM 1,068,000',
      'Planned covered route to the RTS and CIQ within 850 metres',
...
function projectCardHtml(p) {
  ...
  const href = slug ? `/projects/${encodeURIComponent(slug)}` : '#';
  return `
...
async function renderProjectPage(forcedSlug) {
  const params = new URLSearchParams(window.location.search);
  const pathSlug = window.location.pathname.match(/\/projects\/([^/]+?)(?:\.html)?\/?$/);
  const slug = forcedSlug || params.get('slug') || (pathSlug ? decodeURIComponent(pathSlug[1]) : '');
  ...
  root.innerHTML = `
    <header class="project-hero" style="background-image: url('${heroImage}');">
      <div class="project-hero-content">
        <div class="breadcrumb">
          <a href="/">Home</a> &nbsp;/&nbsp; <a href="${areaHref}">${p.area}</a> &nbsp;/&nbsp; ${p.project_name}
        </div>
        <div class="hero-tags">
        ...
        <div class="project-gallery reveal">
          ${p.images.map(img => `
            <figure class="project-gallery-item">
              <img src="${/^https?:\/\//.test(img.url || '') || (img.url || '').startsWith('/') ? img.url : '/' + img.url}" alt="${img.caption || p.project_name}" loading="lazy">
...
const TRANSLATIONS = {
  en: {
    ...
    'hub-sfz': "Forest City SFZ MM2H: What Property Buyers Should Know",
...
function applyLanguage(lang) {
  ...
  const navLinkMap = [
    { hrefs: ['/'], key: 'nav-home' },
    { hrefs: ['/projects'], key: 'nav-projects' },
    { hrefs: ['/articles'], key: 'nav-guides' },
    { hrefs: ['/about-sam-tee'], key: 'nav-about' },
    { hrefs: ['/jb-town'], key: 'nav-jbt' },
    { hrefs: ['/iskandar-puteri'], key: 'nav-ip' },
    { hrefs: ['/forest-city'], key: 'nav-fc' },
  ];
...
const ALL_ARTICLES = [
  // Newest first
  { title: 'Is Forest City Really Empty? What the Network School Closure Actually Means', title_zh: '森林城市真的没人了吗？Network School结束后的真实现况', summary: 'Network School has closed, but Forest City itself remains active. A local look at its beach, family events, commercial activity and the gap between weekend footfall and long-term demand.', summary_zh: 'Network School的计划结束，不代表森林城市停止运作。从本地家庭活动、沙滩、商业街与实际生活情况，看森林城市目前更完整的面貌。', body: 'exists', image_url: 'photos/forest-city/fc-island.png', link: '/articles/forest-city-network-school-current-situation', area: 'Forest City', topics: 'forest-city,buying-guide,other' },
  { title: 'The Kews Iskandar Puteri: Is This Landed Location Right for You?', title_zh: 'The Kews依斯干达公主城：这里的有地住宅适合你吗？', summary: 'A location and lifestyle guide to The Kews near Second Link, compared with Eco Botanic and Sunway Iskandar.', summary_zh: '比较The Kews、Eco Botanic与Sunway Iskandar的地点与生活方式，帮助买家判断是否适合自己。', body: 'exists', image_url: 'photos/sunway-iskandar/sunway-second-link.jpg', link: '/articles/the-kews-iskandar-puteri-landed-near-second-link', area: 'Iskandar Puteri', topics: 'iskandar-puteri,buying-guide,retirement' },
  { title: 'Horizon Hills Cluster Around RM1.6M–RM1.7M: New or Subsale?', title_zh: 'RM1.6M–RM1.7M的Horizon Hills田字屋：新房还是二手？', summary: 'Compare The Peak with older Golf East and Canal Garden South cluster homes by condition, built-up, surroundings, renovation and final total cost.', summary_zh: '从屋况、建筑面积、成熟环境、装修与最终总成本，比较The Peak与Golf East、Canal Garden South的田字屋。', body: 'exists', image_url: 'photos/horizon-hills/hh-3.png', link: '/articles/horizon-hills-cluster-new-vs-subsale-2026', area: 'Iskandar Puteri', topics: 'iskandar-puteri,buying-guide' },
  { title: 'What Can RM1M, RM1.5M and RM2M Buy in Iskandar Puteri?', title_zh: 'RM1M、RM1.5M与RM2M在依斯干达公主城能买什么有地住宅？', summary: 'A practical budget guide to terrace, cluster and Semi-D choices in Iskandar Puteri, and why a higher property label is not always the better family decision.', summary_zh: '按RM1M、RM1.5M与接近RM2M预算，比较排屋、田字屋与半独立式住宅，以及不同家庭应接受的取舍。', body: 'exists', image_url: 'photos/estuari/estuari-home-options.jpg', link: '/articles/iskandar-puteri-landed-property-by-budget-2026', area: 'Iskandar Puteri', topics: 'iskandar-puteri,buying-guide' },
...
async function getProjectsExtended() {
  const extendedSlugs = new Set(EXTENDED_PROJECTS.map(p => p.slug).filter(Boolean));
  const localProjects = [
    ...EXTENDED_PROJECTS,
    ...PLACEHOLDER_PROJECTS.filter(p => p.slug && !extendedSlugs.has(p.slug))
  ];
  const rows = await fetchSheet(SHEET_CONFIG.projectsCsvUrl);
  if (rows && rows.length) {
    const sheetList = rows.filter(r => r.published === 'TRUE');
    const localSlugs = new Set(localProjects.map(p => p.slug).filter(Boolean));
    const sheetOnly = sheetList.filter(r => !localSlugs.has(r.slug));
    return [...localProjects, ...sheetOnly];
  }
...
function projectCardHtmlExtended(p) {
  const bg = p.image_url ? `background-image:url('${p.image_url}')` : '';
  const slug = p.slug || slugify(p.project_name);
  const href = p.project_url ? p.project_url : (slug ? `/projects/${encodeURIComponent(slug)}` : '#');
  return `
